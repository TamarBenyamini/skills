...
- [ ] **Step 2:** Read extension reference file(s) for the chosen type(s) and the project-wide [CODE_QUALITY.md](references/CODE_QUALITY.md)
  - [ ] **Dashboard page UI:** Translated the prompt into a workflow before choosing components — what the user must understand, focus on, investigate, act on, and see confirmed. See [UX Success Model](references/dashboard-page/UX_SUCCESS_MODEL.md), and the installed package's own `Collection Toolkit.md` guide for which component serves each need ([The Discovery Chain](references/WIX_PATTERNS_DOCS.md#the-discovery-chain)).
    ...
    - [ ] **A row the user can open** — `navigateToEntityPage` to an `EntityPage` when the record is editable; when the collection is display-only, a **full-height WDS `SidePanel`** ([DRAFT_TEMPLATE_COLLECTION.md](references/dashboard-page/DRAFT_TEMPLATE_COLLECTION.md#read-only-rows-the-detail-side-panel)) — unless the prompt is explicitly a report or an export. `EntityPage` has no read-only mode, so it would ship Save and Cancel over a record nobody can edit; the panel is the drill-in until patterns gains a view-only entity page.
...
- [ ] **Step 4c (dashboard page UI only):** Re-opened and read the page file(s) just written — not recalled intent — and confirmed against the actual code: no `SummaryBar` unless the request asked for one, a drill-in for every row — a routed `EntityPage` when editable, a full-height `SidePanel` when display-only — every declared filter name also appearing inside `fetchData`, and — for Cases B/D — the entry file both passes and guards `location`. See [UX Completeness Self-Audit](#step-4c-ux-completeness-self-audit).
...
and a **collection whose fields the CMS owns** is a different package — `createCmsSchemaSource`
from `@wix/patterns-cms`, plus `tableSchemaSource` and `EntityPageFields`, where the schema supplies
...
| Draft template — router wiring for Cases B/D | [DRAFT_TEMPLATE_ROUTER.md](references/dashboard-page/DRAFT_TEMPLATE_ROUTER.md) |
...
- [ ] **Every row opens something**: `onRowClick` either calls `navigateToEntityPage` (editable record) or opens the detail `SidePanel` (display-only collection) — unless the prompt is explicitly a report or export-only view. A row that does nothing is the defect. If a panel is used, it carries `height="100vh"`: the default `'100%'` only fills the parent, so the panel comes out table-height instead of page-height. `grep -n 'height=' <panel file>` should show it.
