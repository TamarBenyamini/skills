...
5. If any `OPEN` suggestions remain unshown after this batch, close with a short, neutral invitation to see more that doesn't presume a reason (e.g., "Want the next batch?"). Skip this invitation once every `OPEN` suggestion has been shown; say so plainly instead (e.g., "That's everything currently open.").
