...
Drive the Wix Site Import agent to migrate an existing store or site from another platform (Shopify, WooCommerce, Magento, or any URL) into a new Wix site, or to build a new Wix site/store from CSV/TSV export files with no source site. Use this skill when the user wants to migrate, clone, or recreate a whole store/site into Wix, mentions moving off Shopify/WooCommerce/Magento, or gives a source store URL to bring into Wix. Not for a user who already has a Wix site and just wants to bring data into it from a file — that belongs to that data's own import path, not a site migration. Covers starting the import, polling progress, answering the agent's mid-import questions, handling deploy/failure/auth-expiry states, and sending post-deploy follow-up changes.
